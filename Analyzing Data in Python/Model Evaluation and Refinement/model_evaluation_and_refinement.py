...
# Import LinearRegression from from module linear_module
...
#Print the average standard deviation of the estimate
...
# can also use the function 'cross_val_predict' to predict the output. The function splits up the data into the specified number of folds, using one fold to get a prediction while the rest of the folds are used as test data. First import the function:

from sklearn.model_selection import cross_val_predict
# input the object, the feature in this case 'horsepower' , the target data y_data. The parameter 'cv' determines the number of folds; in this case 4. We can produce an output:
...
#observe how the R^2 changes on the test data for different order polynomials and plot the results:
Rsqu_test = []
...
# DO NOT BE SCARED !!! THIS WILL BE EXPLAINED IN THE UPCOMING SECTIONS!!!
# Take a breath and just run this!
...
#Similarly, prediction can be obtained by:
...
#select the value of Alpha that minimizes the test error, for example, we can use a for loop
...
#lot out the value of R^2 for different Alphas
width = 12
height = 10
plt.figure(figsize=(width, height))
...
#The object finds the best parameter values on the validation data. Obtain the estimator with the best parameters and assign it to the variable BestRR as follows:
